...
SEED_RULES: list[dict[str, str]] = [
    ...
    # ── Mod scan patterns (Kaiserreich, Toolpack, Global Market) ──
    {
        "category": "convention",
        "severity": "style",
        "context": "Focus tree completion logging",
        "context_tags": "focus_trees,completion_reward,debugging",
        "pattern": "Creating focus completion_reward blocks without a log = statement as the first line",
        "correction": "Always put log = \"[GetLogRoot]: Focus Completed <focus_id>\" as the first line in completion_reward. This makes debugging focus tree navigation trivial — you can trace exactly which focuses fired and when.",
        "source": "seed",
    },
    {
        "category": "design",
        "severity": "warning",
        "context": "Focus tree AI navigation with date/flag gates",
        "context_tags": "focus_trees,ai,focus_ai,ai_will_do",
        "pattern": "Using ai_will_do = { factor = X } without date or flag-gated modifiers on historical focuses",
        "correction": "Use factor = 0 modifiers gated by dates (date < 1938.1.1) or country flags to prevent AI from taking focuses too early or in wrong order. Without gates, AI takes focuses as soon as prerequisites are met regardless of historical timing.",
        "source": "seed",
    },
    {
        "category": "convention",
        "severity": "style",
        "context": "Event file ID range pre-allocation",
        "context_tags": "events,namespaces,ids,organization",
        "pattern": "Defining event IDs without pre-allocating ranges in comments at the top of the file",
        "correction": "Add a comment block at the top of each event file showing the ID ranges: # Intro (namespace.0 -> namespace.6), # Black Monday (namespace.7 -> namespace.12). This prevents ID collisions across the team and makes finding events by topic trivial.",
        "source": "seed",
    },
    {
        "category": "convention",
        "severity": "style",
        "context": "Focus search_filters",
        "context_tags": "focus_trees,ui,search_filters",
        "pattern": "Creating focus nodes without search_filters = { } block",
        "correction": "Every focus should include search_filters with relevant tags (e.g., search_filters = { FOCUS_FILTER_POLITICAL FOCUS_FILTER_INDUSTRY }). This makes the in-game focus search functional for players.",
        "source": "seed",
    },
    {
        "category": "design",
        "severity": "warning",
        "context": "Scripted GUI flag-based state management",
        "context_tags": "scripted_gui,gui,flags,state",
        "pattern": "Using complex nested visible triggers instead of flat flag-based state for GUI element visibility",
        "correction": "Use a flat flag-based state machine: each interactive element sets/clears a single global flag. Visibility checks just that flag. Keep the state machine in effects, not in visible triggers. This makes GUI behavior debuggable by checking flags in the console.",
        "source": "seed",
    },
    {
        "category": "convention",
        "severity": "style",
        "context": "Decision categories vs scripted GUI choice",
        "context_tags": "decisions,scripted_gui,design,ui",
        "pattern": "Using decisions for complex interactive systems that would work better as scripted GUIs",
        "correction": "If the mechanic involves tabs, dynamic lists, real-time variable display, or complex multi-step interactions, consider a scripted GUI instead of decisions. Decisions are best for single-action choices with simple costs. Scripted GUIs handle complex state machines, variable economies, and custom UI.",
        "source": "seed",
    },
]
